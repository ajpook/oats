        ...
        static Byte[] ToBinaryImplementation <T> (Object obj)
        {
            ...
            using (var stream = new MemoryStream ())
            {
                    ...
                    (stream, ChannelMode.Write)) 
                {
                    ...
                    // Note that the buffer contains allocated bytes which might
                    // be unused. For example, if the string "test" is written
                    // into the MemoryStream object, the length of the buffer
                    // returned from GetBuffer is 256, not 4, with 252 bytes
                    // unused. To obtain only the data in the buffer, use the
                    // ToArray method.
                    //     stream.GetBuffer ();
